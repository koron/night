...
var CT = require('./content_type.js');
...
function Server(conffile, verbose)
{
  ...
  function loadConfig(filename)
  {
    ...
    if (filename != null) {
      ...
      for (var key in loaded) {
        ...
        } else if (CONFIG[key] !== null && !isTypeMatch(CONFIG[key], loaded[key])) {
  ...
  function handleFileResource(request, response)
  {
    ...
    fs.stat(path, request.domain.bind(function(err, stats) {
      if (err) {
        ...
        var t = CT.guess(path);
        if (t.isBinary) {
          transferBinary(request, response, fs.createReadStream(path), t);
        } else {
  ...
  function transferBinary(request, response, source, typeInfo)
  {
    var param = parseQueryString(request.url);
    // Add emitters to request's domain.
    request.domain.add(source);
    // Prepare HTTP response header.
    var header = {
